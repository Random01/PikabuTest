...
class JobSeeker {
	...
	get seekerCard() {
		return `
			<seeker-card>
				<my-info>
					<my-name>Алексей М.</my-name>
					<my-age>34</my-age>
				</my-info>
				<loc-info>
					(<loc-city>г. Саратов</loc-city>)
				</loc-info>
				<prof-info>
					(<prof-skills>вело, фото, гребля, ловля, ковка</prof-skills>)
				</prof-info>
			</seeker-card>
		`;
	}
	 ...
	 */
	*getMod256(numbers) {
		return yield new Int16Array(numbers);
	}
	...
	findPrimeNumbers(max) {
		...
		let c = document.createElement('someElement');
		...
		let location = gl.getAttribLocation(prg, 'someName');
